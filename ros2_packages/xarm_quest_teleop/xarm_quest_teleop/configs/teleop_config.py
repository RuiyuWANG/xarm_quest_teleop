...
from xarm_quest_teleop.configs.ros2_config import quest_endpoint_cmd, xarm_launch_cmd
from xarm_quest_teleop.configs.robot_config import ROBOT_TOPIC

Hand = Literal["left", "right"]

@dataclass
class TeleopConfig:
    # Autolaunch (subprocess ros2 run/launch)
    auto_launch_quest: bool = True
    auto_launch_robot: bool = True

    QUEST_LAUNCH_CMD: List[str] = field(default_factory=quest_endpoint_cmd)
    ROBOT_LAUNCH_CMD: List[str] = field(default_factory=xarm_launch_cmd)

    scripts_package: str = "xarm_quest_teleop"
