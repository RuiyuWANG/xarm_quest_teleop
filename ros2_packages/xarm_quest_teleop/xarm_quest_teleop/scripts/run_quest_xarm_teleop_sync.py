...
def main():
    ...
    if cfg.auto_launch_robot:
        p = ManagedProcess("xarm_driver", cfg.ROBOT_LAUNCH_CMD, cfg.launch_workdir, cfg.pipe_launch_output)
        p.start()
        procs.append(p)

    # start wrapper nodes
    stamp_quest_cmd = ["ros2", "run", cfg.scripts_package, "quest_stamped_node"]
    ...
    must_srvs = [services.set_mode, services.set_state, services.move_servo_cart,
