# usage: ros2 run xarm_quest_teleop camera_calibration --ros-args -p camera_name:=d405 -p setup:=eye_in_hand
"""
ROS2 Charuco calibration runner (auto-launch robot + ONE selected RealSense).
...
Configured here:
  - RealSense launch cmds from config/ros2/hardware.yaml
...
from xarm_quest_teleop.io.process_manager import ManagedProcess, wait_for_topic, wait_for_service
...
from xarm_quest_teleop.configs.ros2_config import load_hardware_config, realsense_launch_cmd
...
_HARDWARE = load_hardware_config()
REALSENSE_CMDS: Dict[str, List[str]] = {
    name: realsense_launch_cmd(cfg)
    for name, cfg in _HARDWARE.get("cameras", {}).items()
}
...
def main():
    ...
    if robot_cmd:
        p = ManagedProcess("xarm_driver", robot_cmd, workdir, pipe_output)
        p.start()
        procs.append(p)
