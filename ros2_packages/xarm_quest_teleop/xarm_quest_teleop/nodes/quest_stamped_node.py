...
class QuestStampNode:
    ...
    def _stamp_is_zero(stamp) -> bool:
        if hasattr(stamp, "to_sec"):
            return stamp.to_sec() <= 0.0
        return int(getattr(stamp, "sec", 0)) == 0 and int(getattr(stamp, "nanosec", 0)) == 0

    def _stamp_pose_header(self, msg: PoseStamped) -> PoseStamped:
        out = PoseStamped()
        out.header = msg.header
        out.pose = msg.pose
        if self.always_overwrite_pose_stamp or self._stamp_is_zero(out.header.stamp):
            out.header.stamp = self._stamp()
    ...
    def _cb_right_pose(self, msg: PoseStamped):
        self.pub_rp.publish(self._stamp_pose_header(msg))

    def _cb_left_pose(self, msg: PoseStamped):
        self.pub_lp.publish(self._stamp_pose_header(msg))
