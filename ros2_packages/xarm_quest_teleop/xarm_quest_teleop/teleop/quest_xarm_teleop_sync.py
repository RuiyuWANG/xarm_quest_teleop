...
from xarm_quest_teleop.robots.xarm import XArmRobot, XArmState
from xarm_quest_teleop.configs.teleop_config import TeleopConfig
from xarm_quest_teleop.configs.robot_config import GRIPPER_MIN, GRIPPER_MAX

from xarm_quest_teleop.utils.teleop_utils import (
    clamp,
    quat_xyzw_to_rot,
    rot_to_axis_angle,
    wrap_to_pi,
...
class QuestXArmTeleopSync:
    ...
    def __init__(self, cfg: TeleopConfig, quest: Quest2Interface, robot: XArmRobot):
        ...
        # ATS subs
        node = rospy.get_node()
        self.pose_sub = message_filters.Subscriber(node, PoseStamped, pose_topic)
        self.inputs_sub = message_filters.Subscriber(node, OVR2ROSInputsStamped, inputs_topic)
        self.inputs_sub_rest = message_filters.Subscriber(node, OVR2ROSInputsStamped, input_topic_rest)
        self.robot_sub = message_filters.Subscriber(node, RobotMsg, cfg.robot_state)
    ...
    def _ats_cb(self, pose: PoseStamped, inputs_st: OVR2ROSInputsStamped, inputs_st_rest: OVR2ROSInputsStamped, robot_st: RobotMsg):
        ...
        self._latest_sync_stamp = _stamp_to_sec(pose.header.stamp)
