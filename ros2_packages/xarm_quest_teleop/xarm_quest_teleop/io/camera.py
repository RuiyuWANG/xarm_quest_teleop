...
from xarm_quest_teleop.utils.teleop_utils import _stamp_to_sec
...
class _RingImg:
    ...
    def drop_older_than(self, t_min: float):
        while self.buf and _stamp_to_sec(self.buf[0].header.stamp) < t_min:
            self.buf.popleft()
    ...
    def nearest(self, t: float, window: float) -> Optional[Image]:
        ...
        best_dt = 1e9
        for m in self.buf:
            dt = abs(_stamp_to_sec(m.header.stamp) - t)
            if dt < best_dt:
                best, best_dt = m, dt
        if best is None or best_dt > window:
            return None
...
class CamRgbDepthSync:
    ...
    def _on_rgb(self, rgb: Image, name: str):
        t = _stamp_to_sec(rgb.header.stamp)
        with self._lock:
    ...
    def _on_depth(self, depth: Image, name: str):
        t = _stamp_to_sec(depth.header.stamp)
        with self._lock:
...
class TwoRgbSync:
    def __init__(
        ...
        self.subs = [
            message_filters.Subscriber(
                ...
                qos_profile=int(sub_queue_size),
            )
    ...
    def _cb(self, *imgs: Image):
        ts = [_stamp_to_sec(img.header.stamp) for img in imgs]
        t_ref = sum(ts) / len(ts)
