...
class LiveVizRenderer:
    ...
    def _load_task_meta(self) -> dict:
        ...
        calib_path = configured_path if os.path.isabs(configured_path) else os.path.join(repo_root, configured_path)
        fallback_path = os.path.join(repo_root, "config", f"{self.cfg.task_name}.json")

        for path in (calib_path, fallback_path):
