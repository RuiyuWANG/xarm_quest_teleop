...
from xarm_quest_teleop.eval import action_executor as action_executor_module
from xarm_quest_teleop.eval.action_executor import PolicyActionExecutor

action_executor_module.rospy.Time = _FakeTime
action_executor_module.rospy.is_shutdown = lambda: False
action_executor_module.rospy.logwarn = lambda *args, **kwargs: None
action_executor_module.rospy.logwarn_throttle = lambda *args, **kwargs: None
...
def test_reset_clears_executor_state():
    ...
    executor._last_grip_value = 100.0

    executor.reset()

    assert executor.last_exec_action10 is None
    assert executor._last_grip_bin is None
    assert executor._last_grip_value is None


def test_execute_chunk_sends_servo_and_suppresses_duplicate_gripper(monkeypatch):
    ...
    assert robot.gripper_calls == [100.0]
    assert np.allclose(executor.last_exec_action10, chunk[-1])


def test_execute_chunk_scales_meter_actions(monkeypatch):
    _force_ros_running(monkeypatch)
    monkeypatch.setattr(action_executor_module.time, "sleep", lambda _: None)
    robot = _Robot()
    cfg = _Cfg(xyz_unit="m")
    ...
    chunk = np.stack(
        [
            _action((0.10, 0.0, 0.10), 100.0),
            _action((0.11, 0.0, 0.10), 100.0),
        ],
        axis=0,
    )
    ...
    assert ok is True
    assert robot.servo_calls[0][0][:3] == [100.0, 0.0, 100.0]
    assert robot.servo_calls[-1][0][:3] == [110.0, 0.0, 100.0]
